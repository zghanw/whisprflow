...
corrections, matching tone — in under ~700ms. This project rebuilds that same
...
```
   mic audio
       ...
       ▼
┌─────────────────┐     ┌──────────────────────┐
│  faster-whisper  │────▶│   Ollama (local LLM)  │────▶ pasted into
│  large-v3-turbo  │ raw │   qwen3:4b-instruct   │ clean  active window
│  (CUDA, int8)    │text │   (cleanup + tone)    │ text
└─────────────────┘     └──────────────────────┘
                                   │
                                   ▼
                          dictionary.txt
                     (auto-learned jargon/names,
                      fed back into both models)
```
